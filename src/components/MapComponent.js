    ...
    switch(type){
      case "polygon":
        ...
        outer.addListener("mousedown", evt => { 
          // l(evt) 
          if(evt.vertex) { // For editable points on vertices
            let currentShape = findShapeGroupById(currentTag.shapes, shape.shapeId)
        ...
        setNewPath = () => {
          ...
          } else if(!isDragging) { // Because while dragging, this event is continuously emitteed
            this.setState({ 
              undoRedo: { enabled: true, type: "undo", time: 10 }
            }, this.undoRedoTimer)
          ...
          currentShape.undoRedo.outer.path.curr = currentShape.outer.getPath().getArray()
        ...
        if (method === "draw") {
          ...
          outer.setPath(shape.getPath().getArray().map(pt => sph.computeOffset(pt, this.props.influence, getBearing(shape.getCenter(), pt))))
        } else {
        ...
        undoRedo.shape.path = { prev: shape.getPath().getArray(), curr: shape.getPath().getArray() }
        undoRedo.outer.path = { prev: outer.getPath().getArray(), curr: outer.getPath().getArray() }
      ...
      default:
        ...
        shape.addListener("dragstart", () => { 
          ...
          currentShape.undoRedo.shape.ne.prev = currentShape.undoRedo.shape.ne.curr
          currentShape.undoRedo.shape.sw.prev = currentShape.undoRedo.shape.sw.curr
          this.setState({ 
            undoRedo: { enabled: true, type: "undo", time: 10 }
          }, this.undoRedoTimer)
        })
        ...
        shape.addListener("dragend", () => {
          ...
          currentShape.undoRedo.shape.ne.curr = currentShape.shape.getBounds().getNorthEast()
          currentShape.undoRedo.shape.sw.curr = currentShape.shape.getBounds().getSouthWest()
          // l("prev",  currentShape.undoRedo.shape.center.prev.lat(), "curr", currentShape.undoRedo.shape.center.curr.lat())          
        })
        ...
        outer.addListener("bounds_changed", () => {
          ...
          switch(rectEventType){
            ...
            case "fromSelf_drag": // Dragging
              ...
              currentShape.undoRedo.outer.ne.prev = currentShape.undoRedo.outer.ne.curr
              currentShape.undoRedo.outer.sw.prev = currentShape.undoRedo.outer.sw.curr
        ...
        undoRedo.shape.ne = { prev: shape.getBounds().getNorthEast(), curr: shape.getBounds().getNorthEast() }
        undoRedo.shape.sw = { prev: shape.getBounds().getSouthWest(), curr: shape.getBounds().getSouthWest() }
        undoRedo.outer.ne = { prev: outer.getBounds().getNorthEast(), curr: outer.getBounds().getNorthEast() }
        undoRedo.outer.sw = { prev: outer.getBounds().getSouthWest(), curr: outer.getBounds().getSouthWest() }
    ...
    switch(currentShape.type){
      ...
      default: 
        switch(undoRedoObj.action){
          ...
          default: // 'bounds_changed_inner':
            currentShape.shape.setBounds(new mapsApi.LatLngBounds(
